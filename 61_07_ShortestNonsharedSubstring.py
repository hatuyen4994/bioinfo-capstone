...
class Node:
    total = 0
    def __init__(self, isLeaf = True, startIdx = 0, depth = 0, parentEdge = None):
        Node.total += 1
        self.id = self.total
        self.isLeaf = isLeaf
...
class SuffixTree: #Naive algorithm
    def __init__(self):
        ...
        #print(lss)
        sns = self.shortestNonsharedSubstring(self.tree, self.root, self.text, self.l1, self.l2)
        print(sns)
    ...
    def splitEdge(self, edge, startIdx, endIdx, cutIdx, leafLabel):
        ...
        self.tree[newEndNode] = dict()
        edge2 = Edge(cutIdx, edge.endIdx, self.text, newNode, edge.endNode, leafLabel)
        edge.endNode.parentEdge = edge2
    ...
    def shortestNonsharedSubstring(self, tree, root, text, l1, l2):
        # 0: red; a path ending in it spells out a substring that appears in text2 but not in text1
        # 1: blue; a path ending in it spells out a substring that appears in text1 but not in text2
        # 2: purple; a path ending in a purple node in the suffix tree of Text1 and Text2 spells out a substring shared by 
        #    Text1 and Text2
        ...
        for v in post:
            if v.isLeaf:
                color[v] = 0 if v.parentEdge.startIdx > l1 else 1
            else:
                ccolors = [color[w.endNode] for w in tree[v].values()]
                if 1 == len(set(ccolors)):
                    color[v] = ccolors[0]
        ...
        # Find shortest nonshared substring
        best = (None, np.inf)
        for v in post:
            if 1 != color[v] or None == v.parentEdge:
                continue
            if v.isLeaf:
                leafLength = l1 - v.parentEdge.startIdx # length of text1 part at leaf edge
                if leafLength > 0:
                    currLength = v.parentEdge.startIdx+1-v.startIdx
                    if currLength < best[1]:
                        best = (v.startIdx, currLength)
            else:
                currLength = v.depth+1-v.parentEdge.length()
                if currLength < best[1]:
                    best = (v.startIdx, currLength)          
        if best[0] != None:
            return text[best[0]:best[0]+best[1]]
